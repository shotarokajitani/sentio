...
> **2026-08-20 追記。正本は `docs/runbooks/2026-08-20_service-role-rotation.md` に移した。**
> **下の6手順は不完全である。** レガシー JWT のローテートは `anon` キーも同時に変えるため、
> 実際に触る先は3箇所ではなく**7箇所**ある（Vercel の `SUPABASE_ANON_KEY` と
> `SUPABASE_SERVICE_ROLE_KEY`、Edge Function の `SUPABASE_ANON_KEY` が漏れている）。
> さらに**発行済みの全ユーザーセッションが無効になる**（強制ログアウト）。
> 実施するときは必ず上記 runbook を使うこと。下は経緯の記録として残す。
