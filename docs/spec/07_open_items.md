...
**一時的ではなく、続いている状態である。** 2026-08-18 のセッション記録にも同型の実測がある
——**物理 7.8GB に対し空き 0.6GB、コミット上限 22.5GB に対し空き 0.8GB**で、
Stop hook が `uv_spawn` で失敗していた（原因は hooks ではなくプロセス生成の失敗）。
主因は当時の実測で chrome 47プロセス 7.7GB ＋ claude 13プロセス 3.7GB だった。

> 検収側からは「2026-08-21 の記録にも空き 0.3GB がある」と指摘があったが、
> **その数値はリポジトリ内には見つからなかった**（`grep` で0件）。
> ここに引いたのは 2026-08-18 のセッション記録の実測値である。
