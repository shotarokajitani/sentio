...
**2026-09-01 追記: D2 も同じ形にした（この判断の適用漏れを直す）。**
D2 は `toBeLessThanOrEqual(2)`、すなわち**合格線のまま**受けていた。
その形では**誤検知が 1 や 2 に増えても緑のまま通る**——
「上振れも赤にして現在地の更新を強制する」という上の判断が、D1 にしか掛かっていなかった。
実測値 0 件を確認したうえで `expect(count).toBe(0)` に固定する。
増えたら赤になり、**なぜ増えたかを確かめてから現在地を更新する**ことになる。
...
>
> **2026-09-02（4）: 6 → 7。シリーズ単位の間隔を見る走査を足した（梶谷さん判断）。**
> イベントを定例の名前・取引先で束ね、系列ごとに「途絶」と「伸長」を見る。
> **両方が同じ間隔列から出る**ので、1つの仕掛けで `#1` と `#7` が同時に埋まった。
> 会社全体の途絶（走査6）は**残してある**——「全部止まった」は系列に束ねられない形なので、
> シリーズ版では捉えられない。2つは別の信号である。
>
> あわせて仕込み `#1` を直した。**名前と逆の形になっていた**（間隔が縮み、売上が増える
> データだった）。golden の記述「発注間隔が3ヶ月かけて伸長」とも食い違っていた。
