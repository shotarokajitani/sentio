...
--   バックアップは取得予定だったが、2026-08-18 に取り違えで未取得と判明（材料喪失）。
--   旧DBの実データはテスト会社1件のみ（実ユーザーゼロ）と削除前に確定しており、
--   影響なしとして検収済み。Supabase Pro の日次バックアップに復元ポイントは残る。
--
...
--   （2026-08-20 追記: この型＝SQL自体が落ちる故障は job_run_details に出る。
--    一方 net.http_post は非同期なので、Edge Function が 401/500 を返す故障は
--    そこに出ない。関数側の成否は net._http_response で見ること）
--
-- ---------------------------------------------------------------------------
