...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const [summaryRes, baselinesRes, eventsRes, findingsRes, narrativesRes] =
      await Promise.all([
        supabase.from("company_summary").select("content")
          .eq("company_id", company_id).single(),
        supabase.from("baselines").select("metric_key, is_established, median, iqr")
          .eq("company_id", company_id).eq("is_established", true),
        supabase.from("events").select("event_id, event_type, source, metrics, occurred_at")
          .eq("company_id", company_id)
          .order("occurred_at", { ascending: false }).limit(20),
        supabase.from("findings").select("what, urgency, status, evidence_event_ids")
          .eq("company_id", company_id).in("status", ["open", "watching"]),
        supabase.from("narratives").select("content, confidence")
          .eq("company_id", company_id)
    ...
    const rawSections = [
      ...
      { type: "baselines", priority: 2, content: (baselinesRes.data || [])
        .map((b: { metric_key: string; median: number; iqr: number }) =>
          `${b.metric_key}: median=${b.median}, iqr=${b.iqr}`
        ).join("\n") || "(no baselines)" },
      { type: "recent_events", priority: 3, content: (eventsRes.data || [])
        .map((e: { event_type: string; source: string; occurred_at: string }) =>
          `[${e.event_type}] ${e.source} @ ${e.occurred_at}`
        ).join("\n") || "(no recent events)" },
      { type: "findings", priority: 4, content: (findingsRes.data || [])
        .map((f: { what: string; urgency: string }) => `[${f.urgency}] ${f.what}`)
        .join("\n") || "(no open findings)" },
      { type: "narratives", priority: 5, content: (narrativesRes.data || [])
        .map((n: { content: string; confidence: number }) =>
    ...
    return new Response(
      JSON.stringify({ status: "ok", packet }),
