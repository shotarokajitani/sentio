...
initSentry("ga4-oauth");

// GA4 は google-calendar と同じ Google OAuth クライアントを共有する。
...
async function exchangeCode(code: string) {
  ...
  if (!r.ok)
    throw new Error(`ga4 token exchange failed: ${r.status} ${await r.text()}`);
...
async function refreshToken(refresh: string) {
  ...
  if (!r.ok)
    throw new Error(`ga4 token refresh failed: ${r.status} ${await r.text()}`);
  return await r.json();
}
...
serve(async (req) => {
  ...
  try {
    if (action === "authorize") {
      ...
      if (authError || !user) {
        console.error("[ga4-oauth] auth failed:", authError);
        return jsonResp(
    ...
    if (action === "callback") {
      ...
      if (error) {
        return Response.redirect(
          `${APP_BASE}/app.html?integration=ga4&result=error&reason=${encodeURIComponent(error)}`,
          302,
        );
      }
      if (!code || !state)
      ...
      return Response.redirect(
        `${APP_BASE}/app.html?integration=ga4&result=success`,
        302,
      );
    }
