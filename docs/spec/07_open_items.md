...
### 2026-09-07 実装 — ④-a（PR #96）。**解約の導線そのものは、まだ無い**
...
**BU-D2（画面から Stripe API を叩かない）は有効のまま**である——webhook は画面ではない。
...
2. **Preview 側に `STRIPE_SECRET_KEY` があるかは未確認である。**
   ...
   **Preview で通しの検証をするときに詰まる。**
3. **運用宛メールの送信経路が未検証である。**
   `SENTIO_OPS_EMAIL` は設定済み（2026-09-07・梶谷さん）だが、
   **未解決が0件のあいだメールは1通も出ない。** したがって
   **「0件だから静か」と「経路が壊れていて静か」を区別できない。**
   `dispatch-daily` の summary に `billing_alert` が出るので
   `not_needed` / `not_configured` / `failed` の**区別はつく**が、
   **Resend が実際に届けたかは一度も確かめていない。**
   本番の `billing_webhook_unresolved` に手で行を入れて試すことは**しない**（本番は読み取りのみ）。
   検証方法は**人間の判断待ち**（選択肢は 2026-09-07 の報告に出した）。

4. **別イベントへの 5xx が invoice 系に波及するかは未確認である。**
   2026-09-07 から、`customer.subscription.*` の `retrieve_failed` / `lookup_failed` にだけ
   503 を返すようにした（Stripe に再送させて人手なしで直す狙い。**副次的に、失敗が続けば
   Stripe から「webhook が失敗している」通知が届く＝気づく経路がもう1つ増える**）。
   Stripe の文献には「`invoice.created` へ成功応答が返らないと、自動収納の請求ファイナライズが
   最大72時間遅れる」とある。**別イベントの 5xx がそこに波及するかは読んでいない。**
   **波及するかに関わらず**、5xx の対象を `customer.subscription.*` に限定してあるので
   この危険は消えている。**「影響しない」とは書かない。未確認である。**

**いずれも「未確認」である。「問題ない」とは書かない。**

**残る限界を3つ、ここに顕在化させる。**

1. **`billing_webhook_unresolved` は `company_id` を持たない。**
   会社が引けなかった事実を記録する表なので持ちようがない。したがって
   **`check:deletion-coverage` の射程外**で、アカウント削除でこの表の行は消えない
2. **気づくのは1日1回である。** 即時に鳴らす経路は本番に無い（「本番の可観測性が実質ゼロ」）
3. **`SENTIO_OPS_EMAIL` は 2026-09-07 に設定済み**（Supabase の Edge Function secrets・
   梶谷さん実施）。未設定に戻れば `dispatch-daily` は
   `"billing_alert":"not_configured"` を返して non-2xx になる（**黙って0件に見せない**）。
   ただし**送信経路そのものは未検証である**（上の未確認3）
...
#### 2026-09-09（①-2）で意味が変わった点

**`reauth_required` が「1回でも失敗した」ではなくなった。**

| いつ         | `reauth_required` の意味                                                             |
| ------------ | ------------------------------------------------------------------------------------ |
| 〜2026-09-09 | **1回でも失敗した**（ネットワークの瞬断・5xx・429 を含む）                           |
| 2026-09-09〜 | **3回続けて失敗した**、または認可が本当に通らない（400/401 で `invalid_grant` 以外） |

読み替えが要るのは3つある。

1. **`status` だけでは失敗の回数が分からない。** `consecutive_failures` を併せて見る。
   1〜2 の行は `active` のままで、**すでに失敗している**
2. **`reauth_required` は「もう触られない行」ではなくなった。**
   `sync-connections` が1日1回試し、成功したら `active` に戻す
   （`last_failure_at` から24時間。以前は `status = 'active'` で絞っていたので二度と触られなかった）
3. **勝手に直った行が出る。** `connection_events` の `reason` で区別する——
   `reconnected` は人が画面から繋ぎ直したとき、`recovered` は cron が自動で戻したとき

**判断が要る点が1つ残っている。** 400 / 401 / 408 / 429 / 5xx 以外の状態コード
（402 / 403 / 404 など）を `transient` に倒した。相手側の設定やプロキシの都合で出ることがあり、
**認可の失敗と断定できない**ためである。3回続けば `reauth_required` に落ちるので18時間で気づくが、
**この割り当ては発注書に無く、こちらの判断である。**
...
```
$ node -e "os.freemem() / os.totalmem()"
300 MB free / 8017 MB total

$ pnpm typecheck
FATAL ERROR: Committing semi space failed. Allocation failed - JavaScript heap out of memory

$ pnpm exec vitest run tests/unit/login-entry.test.ts   → 同じく exit 134 で異常終了
（PreToolUse フックの node も `VirtualAlloc failed` で落ちた）
```
...
> あるが、**この一節はそちらには含まれていない**（`grep` で0件・2026-09-08 確認）。
> `claude/Sentio_CC発注書_20260821.md` §6-3 も同じ数値を引いているが、
> **一次の出所は 2026-08-20 のほう**である。

**3週間にわたって同じ状態が続いている。**

**Docker を入れ直すかどうかの前に、メモリが足りるかがある。**
Docker Desktop は 2026-08-21 にアンインストールされている（上の項目）。
**空きが 300 MB の状態では、入れ直しても動かない。** 順序としてはメモリが先である。
**どちらも判断しない。**

**当面の運用は「CI で確かめる」。** `verify` は `pnpm typecheck && pnpm lint` と
同じコマンドを走らせるので、結論はそちらで取れる。
**ローカルで回せなかったことは、報告に必ず書く**（隠すと「確認済み」と読まれる）。

## `/register` は到達不能な着地ページである（2026-09-08 登録・**判断は書かない**）

**実測。** `src/app/register/page.tsx`（42行）は**登録フォームを1つも持たない。**
中身は見出し・リード2行と、ボタン1つ（「会社情報を接続する」）で、
遷移先は `companyId ? "/connect" : "/login?next=%2Fconnect"`。**登録ではなくログインへ送る着地ページ**である。
文言も `register.title` が「見えない変化・予兆に、気づける」で、登録フォームの見出しではない。

**リポジトリ内に `/register` へのリンクは1本も無い**（`grep` で0件）。到達経路が無く、実害は出ていない。

**`/register/complete` のほうは生きている。** Google 連携の完了後に
`src/app/auth/callback/google/route.ts:121` が `?events=N` 付きで飛ばし、
`src/middleware.ts:5` の保護対象にも入っている。**名前が似ているだけで、別物である。**

登録の実装は `/login` ＋ `/api/auth/session`（`intent=signup`）の**1本だけ**で、二重実装ではない。
**消すか、使うか、名前を変えるかは判断しない。**

## アカウント削除（②）の洗い出し（2026-09-08・**判断は書かない**）

privacy §6 の原文から足りないものを洗い出した。**手順そのものは揃っている**
（`docs/runbooks/2026-08-20_account-deletion.md` の8手順・12テーブル）。足りないのは別のところにある。

### 1. 台帳が無い（**依頼から30日を数える場所**）

§6 は「**ご依頼から30日以内に**」削除すると公開している。いま期限を持っているのは
**依頼メールのスレッドだけ**で、**未処理の依頼が何件あるかを機械で数える経路が無い**。
手順書の「実施記録」は現在「未実施」のままである。
**作るかどうか、どう作るかは判断しない。**

### 2. 証跡が半分しか残らない

手順書は**日付・実施者・本人確認の方法**を人が追記する形になっている。
一方**件数は「依頼メールのスレッドに控える」だけ**で、リポジトリにも DB にも残らない。

**今日、逆転が生まれた。** `retention-purge` は `retention_purge_runs`（00030）に
**対象件数 / 実削除件数 / 判定**を機械で残すようになった。
**自動の削除が機械の記録を持ち、人手のアカウント削除が持たない。**

### 3. 受付窓口の到達が未確認

宛先は `support@mdc-diseno.com`（`privacy` の §6 と §9。`/terms` には連絡先が無い）。

- **ドメインはメールを受ける**（実測: `mdc-diseno.com MX preference=1, mail exchanger=smtp.google.com`）
- **ただし support@ 宛が人に届くかは未確認**。
  `docs/runbooks/2026-08-18_slice-a-cutover.md:197` に
  「`support@mdc-diseno.com` は受信可能を確認済み」（2026-08-19・検収者）とあるが、
  **どう確かめたかは書かれていない**
- 2026-09-08 に梶谷さんの Gmail を検索したところ、**support@ 宛のスレッドは0件**
  （`deliveredto:` / `to:` を trash・spam 含めて検索）。このアカウントの受信履歴からは裏が取れない

確かめ方は2つ。**(a) 1通送って着信を見る**／**(b) Workspace の管理画面で転送先を見る**。
**どちらも実行していない。梶谷さんの作業になる。**

### 4. 外部サービスに残る記録

**Stripe に残る顧客・支払いの記録は消えない。**
`docs/checklists/deletion-coverage.yml` の「守れない範囲」に既に明記がある
（外部サービス（Stripe / Resend）に残る記録は検査の射程外）。
§6 の「すべてのデータ」との関係は**未判断**。

### 5. 検査器の射程を広げるか（**未判断**）

`check:deletion-coverage` が見るのは `company_id` を持つテーブルの列挙だけである。
`stripe_customer_id` のような**company_id を持たない紐づけは見ない**。
2026-09-08 に `billing_webhook_unresolved` を宣言（`beyond_company_id`）＋手順書の1経路で埋めたが、
**宣言と実物の突合はしていない**——「消す SQL が本当にその会社の行だけを消すか」は射程の外にある。
**広げるかどうかは決めない。**
...
**その範囲で片付かなかったものを3つ、ここに出す。**

### 1. `SourceRow` と会計CSVの行で、見せ方が揃っていない

連携カードは `SourceRow`（Google カレンダー / freee の2つが共有）で組んでいるが、
**会計CSVの行は同じ見た目を手書きで複製した別実装**である（`connect-client.tsx:369-470`）。
今回 `SourceRow` を直したので、**CSV 行だけが揃っていない状態になった。**

寄せなかったのは、**CSV が「連携が切れる」という状態を持たない**ためである
（接続ではなくアップロードで、状態は idle / analyzing / confirm / ingesting / done / error の6つ）。
性質の違うものに同じ見せ方を無理に寄せると、**存在しない状態を表現することになる。**

**揃えるかどうかは未判断。** 「問題ない」ではなく「揃っていない」と書いておく。

### 2. freee は API で解除できるのに、画面に導線が無い

`POST /api/connections/disconnect` は `provider` を受け取る形で、**freee でも動く**
（`sourcesForProvider` が `freee` を知っている）。にもかかわらず、
`connect-client.tsx:354` は freee の行に解除 UI を渡していない
（契約の非スコープ「他 provider の解除 UI」による）。

**「できるのに導線が無い」はこれで2つ目である。** 1つ目はアカウント削除
（上の「アカウント削除APIの実装」）。**ローンチ前に判断が要る。**
...
OAuth のコールバックは `status: "active"` を入れて upsert し
（`auth/callback/google/route.ts:103` / `freee/route.ts:101`）、
トークン更新は `active` / `revoked` / `reauth_required` を書く
（`_shared/token-refresh.ts:211,263,288`）。本番の実データにも `pending` は0件。

つまり **`pending` は「DB の既定値としてだけ存在する状態」**である。
画面は現在これを「接続済み」に吸わせている（`cardActions` の既定分岐）。
...
### 判断待ちだった4項目は決まった（2026-09-09・検収者）

| 項目       | 決定                                                                     |
| ---------- | ------------------------------------------------------------------------ |
| 販売価格   | **月額 30000 円（税込）。** 本番 Stripe の既存の商品と価格をそのまま使う |
| 支払方法   | クレジットカード（Stripe）。**一回払いの商品は作らない＝分割払いは不可** |
| 支払の時期 | **無料期間14日の終了日に初回。** 以後は毎月、同じ日                      |
| 返金の条件 | **日割り返金はしない。** 解約は `at_period_end` のまま                   |

**無料期間は Stripe の price ではなく Checkout セッション側で付ける**
（`subscription_data.trial_period_days`）。price に付けるとその価格を使う全員に効き、
変えるには**本番の価格オブジェクトを書き換える**ことになる。

`/legal` は 2026-09-09 に作った。**ただし窓口のアドレスが未設定の間は 404 を返す**（下記）。

## LP の差し替えで「Sentio について」の節が消えた（**未判断**・2026-09-09 登録）

**2026-08-19 に Google の OAuth ブランディング審査から
「ホームページでアプリの目的が説明されていない」と指摘され、その備えとして
LP に「Sentio について」の節（見出しに平文のアプリ名＋事実記述の2段落）を置いていた。**

**2026-09-09 の7ブロック差し替えで、この節は無くなった。** 承認された文言に
該当する節が無いためで、勝手に足していない。**外した事実だけをここに残す。**

- 機械可読なアプリ名は **`<title>` だけ**になった（`app/layout.tsx` の `metadata.title`）。
  ワードマークは `<span>` に割れているので、連続した "Sentio" としては読めない
- 対応する試験（見出しに平文のアプリ名が入ること）は `tests/unit/landing.test.ts` から外した。
  代わりに `<title>` を見る試験を置いた
- **審査は 2026-09-02 に承認済みで、ホームページの本文は同意画面の設定ではない。**
  したがって`.claude/rules/oauth-consent-screen.md` の「引き金」には当たらない。
  **ただし再審査になったとき（スコープを増やしたとき）は、同じ指摘を受けうる**

**どうするかは検収者の判断待ち。** 選択肢は3つある。
...
- 画面（LP / `/legal` / 申込前の確認）が出す金額は**定数**から来る
- 実際に請求される金額は**Stripe の price** から来る

**ずれても自動では気づけない。** Stripe 側を書き換えても CI は緑のままで、
**画面には古い金額が出続ける。** 逆に定数だけ変えると、**表示と請求が食い違う**——
特商法の表示義務の観点では、これは「表示した価格と違う額を請求した」ことになる。

**`SENTIO_SITE_ORIGIN` と同じ形の負債である**（本番の値とコードの想定が別々にあり、
食い違いを機械で見ていない）。**いまは人間が両方を見るしかない。**
突合の自動化（起動時に Stripe の price を1回読んで定数と比べる等）は**未判断**。
読むだけでも本番 Stripe への通信が増えるので、勝手には入れない。

## `/legal` は窓口のアドレスが入るまで 404 を返す（2026-09-09・仕様）

**これは不具合ではない。** `SENTIO_SUPPORT_EMAIL` が未設定なら
`/legal` は `notFound()` で 404 になり、LP と申込前の確認からのリンクも出ない。

**届かないアドレスを法定の表記に載せないため**である（DR-1 が未了）。
検収者の 2026-09-09 の実測では、`support@sentio-ai.jp` と `support@mdc-diseno.com` の
どちらも**バウンスも返らず受信箱にも届いていない**。「不達ではないが、読む人がいない」状態である。

**値そのものはここにもチャットにも書かない。**

`/legal` は `force-dynamic` にしてある（2026-09-09）。
**env を読む面を静的生成すると、ビルド時点の値が焼き付き、変更が再デプロイまで反映されない。**
`/legal` の場合は「未設定でビルド → 404 が焼き付く → 値を入れても 404 のまま」になる。
**`/legal` 以外に env を読む面を作るときは、`force-dynamic` の要否を必ず確認すること。**
トップのフッターのリンクは静的のままにしてある。**ずれる向きを
「ページは出るが、リンクがまだ出ない」側に限る**ためで、逆は起こさせない。

なお Vercel は環境変数の変更を動いているデプロイには反映しない。
**値を入れたあとは再デプロイが要る**（`force-dynamic` が担保するのは焼き付けないところまで）。
