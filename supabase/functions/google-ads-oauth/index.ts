...
initSentry("google-ads-oauth");

// Google Ads は google-calendar / ga4 と同じ Google OAuth クライアントを共有する。
...
async function exchangeCode(code: string) {
  ...
  if (!r.ok)
    throw new Error(
      `google-ads token exchange failed: ${r.status} ${await r.text()}`,
    );
...
async function refreshToken(refresh: string) {
  ...
  if (!r.ok)
    throw new Error(
      `google-ads token refresh failed: ${r.status} ${await r.text()}`,
    );
  return await r.json();
}
...
serve(async (req) => {
  ...
  try {
    if (action === "authorize") {
      ...
      if (authError || !user) {
        console.error("[google-ads-oauth] auth failed:", authError);
        return jsonResp(
    ...
    if (action === "callback") {
      ...
      if (error) {
        return Response.redirect(
          `${APP_BASE}/app.html?integration=google_ads&result=error&reason=${encodeURIComponent(error)}`,
          302,
        );
      }
      if (!code || !state)
      ...
      return Response.redirect(
        `${APP_BASE}/app.html?integration=google_ads&result=success`,
        302,
      );
    }
