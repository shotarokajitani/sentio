...
### 併せて残す — `unpaid` の扱いは未判断

2026-09-08 の決定で 409 の対象にしたのは **`active` / `past_due` / `trialing` の3つ**である。
`unpaid` は**そのまま購読ボタン側に残っている**（`BLOCKS_NEW_CHECKOUT` に入れていない）。

Stripe の `unpaid` は「`past_due` のリトライが尽きた後」であり、
**購読そのものは Stripe 側に残っている。** 理屈だけを言えばポータル側だが、
**決定に含まれていない値を勝手に足さない。** 試験（`tests/unit/billing-checkout-guard.test.ts`）は
**いまの実装のとおり**「`unpaid` は 409 にしない」を固定してある。
**変えるなら、この項目を先に決めること。**
