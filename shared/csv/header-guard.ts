 ...
 *
 * この事故の本体は「**防いでいるつもりだった**」ことなので、
 * どこから守れないかをここに顕在化させる。黙って残した穴は「守られている」と誤読される。
 *
 * 1. **前置き行のあるCSVは検出しない。**
 *    1行目がタイトル（`○○銀行 入出金明細` など）で2行目が列名の形は、
 *    1行目が「列名らしい」と数えられるので**通る**。通れば銀行名がプロンプトに載る。
 *    列数の食い違い（1行目だけセルが極端に少ない）で検出できるが、
 *    **それは CH-D4 とは別の判定**であり、このスライスの範囲外である
 * 2. **カナ中心のデータ行は通りうる。**
 *    半角カナを判定に使わない（CH-D5）ので、数値・日付・空が半数に満たない
 *    データ行——たとえばカナの名義が並ぶ形——は列名の行と見なされる。
 *    実物は 17列中14 が該当したので足りているが、**全銀協の全ての変種で足りる保証はない**
 * 3. **2行目以降は一切見ない。** 1行目だけで判定する。
 *    データ行と突き合わせれば精度は上がるが、突き合わせる対象もまた PII である
 *
 * 1 と 2 に当たる実物が出たら、閾値をいじるのではなく**別の判定を足す**こと。
 * 閾値を下げると CH-2 系（誤検知を出さない）が先に壊れる。
 */
