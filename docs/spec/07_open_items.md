...
**この条件は今日時点で満たされていない。** 実例が観測されていないので、
着手可能かどうかを判断する材料がまだ無い。

## ポリシー §7 の「最大1時間程度」と、cron の6時間おきが食い違っている（未判断・2026-09-03 登録・**公開済みの約束あり**）
...
**どちらが正しいかは、実測を見てから決める。**

### 2026-09-03 に実測する予定

`revoked` の実例で、**取り消しから画面に反映されるまでの実時間**を測る。
上の D-3 の着手条件（`revoked` の実例を人間が確認する）と**同じ機会で取れる**。

- 取り消した時刻と、`connections.revoked_at` が入った時刻の差
- それが「最大6時間」の想定どおりか（cron の直後に取り消せば最長に近い値が出る）

**実測前に文言も cron も変えない。** いま (a) に倒すと、
測って初めて分かる値（実際には何時間かかったか）を反映し損ねる。
