...
### 未判断 — `incomplete` / `paused` / 知らない状態の**文言**（2026-09-08 登録）

関門と「購読ボタンを出さない」ところまでは決まった。**画面に出す状態の文言は未決**である。

いまは `active` → 「標準プラン・購読中」、`past_due` / `unpaid` → 「お支払いを確認できていません」、
**それ以外（`trialing` / `incomplete` / `paused` / 知らない状態）は「試用中」に落ちる。**
`incomplete` に「試用中」と出すのは正確ではないが、
**検収者が「実装を見てから決める」と留保している**ため、いまは変えない。

補足の1行も同様に「解約もこちらから行えます。」に落ちる。
`incomplete`（決済が確定していない）でこの文が正しいかは**未検証**である。
