...
const DAY_RULE = { route: "test", windowSeconds: 86_400, limit: 5 };
...
describe("何回目で止めるか", () => {
  ...
  it("止めたときは窓の終わりまでの秒数を返す（UTC 10:00 → 翌 00:00 まで 14 時間）", () => {
    const d = decideRate(6, DAY_RULE, now);
    expect(d).toEqual({ allowed: false, count: 6, retryAfterSeconds: 14 * 3600 });
  });

  it("窓の終わりの直前でも Retry-After は 1 秒以上", () => {
    const d = decideRate(6, DAY_RULE, new Date("2026-09-13T23:59:59.900Z"));
    expect(d.allowed === false && d.retryAfterSeconds).toBe(1);
  });

  it("10分の窓: 10:07 は 10:00 の窓に入り、残りは 3 分", () => {
    const rule = { route: "s", windowSeconds: 600, limit: 30 };
    const at = new Date("2026-09-13T10:07:00Z");
    expect(rateWindowStart(at, 600).toISOString()).toBe("2026-09-13T10:00:00.000Z");
    expect(decideRate(31, rule, at)).toEqual({ allowed: false, count: 31, retryAfterSeconds: 180 });
  });
});

describe("429 の応答", () => {
  it("status 429 と Retry-After を持つ", async () => {
    const res = tooManyRequests({ allowed: false, count: 6, retryAfterSeconds: 120 });
