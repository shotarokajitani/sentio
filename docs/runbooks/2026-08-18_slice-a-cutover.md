...
| 6. 法務文面の確認                 | **完了**（2026-08-19・下記。草案表示は撤去済み）       |
...
`/privacy` と `/terms` は当初**草案**として実装し、画面上に草案である旨を表示していた。

**2026-08-19 に正式版として確定。** 検収者から次の2点の連絡を受けて草案表示を撤去した。

- `support@mdc-diseno.com` は受信可能を確認済み
- **外部の法務確認は行わず、運営者判断で正式版として確定する**

したがって `legal.draftNotice` と両ページの表示ブロックは削除済み。
文面そのものは変えていないため `UPDATED_AT` は 2026-08-19 のまま据え置き。

> この確定は運営者判断であって、弁護士等による法務レビューを経たものではない。
> 有償化・実ユーザー受け入れの前には、特商法表記（`docs/spec/07_open_items.md`）と
> あわせて改めて外部確認を検討すること。
