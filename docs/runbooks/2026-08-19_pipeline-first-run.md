...
Body（`email` は**検収者のアドレス**）:

```json
{
  "company_id": "197f2c0e-aef8-405d-afcc-34d23c771fcd",
  "email": "shotaro.kajitani@mdc-diseno.com"
}
```
...
1. **不具合#1・#2 を直してから cron に載せるのか、載せないのか。**
   壊れているFunctionをcronに登録すると、`cron.job_run_details` を見ない限り
   誰も気づかない静かな失敗が毎晩積み上がる（`00021` のヘッダに記録された既往の型）
