...
## API ルートの認証が宣言と揃っていない（未判断・2026-08-31 登録・スライスCH の作業中に発見）
...
## 途絶（沈黙）をシリーズ単位で見るか（未判断・2026-09-01 登録）

2026-08-31 に途絶を本番へ実装したが、**会社全体の予定間隔**しか見ていない
（`entity_id = null`）。捉えるのは「**会社の予定が丸ごと途絶えた**」であって
「**毎週の定例が消えた**」ではない。

**会議が密な会社では、平常の間隔が1日前後になるためほぼ発火しない。**
すなわち製品が約束している「報告されない問題が、見える」の中心的な例が、
いまの実装では取れていない。

### なぜその場で作らなかったか

仕様（`docs/spec/02_state.md`）は「指標×**エンティティ(任意)**×粒度」を許しており、
定例シリーズごとに持つのが本来の形である。しかし `baselines.entity_id` は **UUID** で、
`entities` 行の生成が要る。**シリーズの同定キーを何にするか**（カレンダーの
タイトルか、`recurringEventId` か）が決まっていない。

2026-08-31、梶谷さん判断で「まず会社全体を作る」を選んだ。

### 判断が要ること

1. **シリーズの同定キー。** 取り込みが保存しているのは `metrics: { title, attendees }` である。
   タイトルは変わりうる（「定例MTG」→「週次定例」）。変わったら別シリーズになる
2. **`entities` 行をいつ作るか。** 予定を取り込むたびか、夜間のバッチか
3. **合格線の読み替え。** いまの 6/7 は会社全体の途絶で取っている。
   シリーズ版を入れると仕込み `#7` の意味が変わるので、評価の期待も一緒に動く

**合格線に届いたことを「検知が十分になった」と読まないこと。**
走査（`_shared/scan.ts` の走査6）・評価スイート・契約の実測記録の3箇所に明記してある。

## 発注間隔の伸長を検知するか（未判断・2026-09-01 登録）

評価の仕込み `#1 order_interval_elongation` は**唯一まだ検知できていない**（6/7 の残り1件）。

仕込みは同一取引先からの発注が 14 → 18 → 25 → 35 → 50 日と伸びる形である。
**これを見る検出器が存在しない。** 本番の走査は
乖離（売上の平常レンジ逸脱）/ 傾向（連続N期同方向）/ 途絶（予定の間隔）/ 期日 / 外部着火 で、
**取引の間隔を見るものが無い。**

売上の傾向検知で代用できないことは実測済みである（正常データと時系列で混ざり、
連続判定が途切れる。2026-08-31）。

### 判断が要ること

1. **取引先をどう同定するか。** 途絶のシリーズ版と**同じ問題**である（`entities` が要る）。
   まとめて決めた方が筋が良い
2. **仕様の5走査に足すのか、途絶の応用と見るのか。**
   仕様の「途絶」は予定の間隔だが、発注間隔も構造は同じである
3. **7/7 を目指すのか。** 合格線は6であり、既に達成している。
   ここを埋めるのは合格のためではなく、**実データで効くか**の判断であるべきである

## `/api/competitors/suggest` に呼び出し元が無い（未判断・2026-09-01 登録）

**認証あり・Anthropic API を叩く・`entities` を書く**エンドポイントだが、
**呼び出し元が1つも無い。** `git log -S` で探しても存在した形跡が無く、
削除した `/api/analyze-url` と**同じ 2026-07-23 のコミット**
（`feat: connection hub UI, OAuth, CSV drop zone, analyze-url`）で生まれている。

### 影響（**製品の話である**）

`type: "competitor"` の entity を書いているのは**このエンドポイントだけ**であり、
`day0` はそれを読む側である（`fetchCompetitors`）。したがって——

> **Day0 レポートの競合の節は、一度も実データで埋まったことがない。**
> `competitorsSummary` が常に空になり、プロンプトには「推定なし」が入る。

Scanner と同じ形である。**実装はあるが、起動されない。**

### 判断が要ること

1. **消すか、呼び出し元を作るか。** 消せば `analyze-url` と同じで面が無くなる。
   作るなら「いつ競合を推定するか」（登録時か、Day0 の直前か、手動か）を決める
2. **作る場合の費用。** 1回あたり Anthropic API と gBizInfo API を叩く

**`check:endpoint-callers` の宣言には載せていない。**
載せると `no-caller` で赤くなり、判断が済むまで本番へ何も出せなくなるためである
（`deploy.yml` の verify が落ちると `deploy-migrations` に到達しない）。
**判断が済んだ時点で、消すか宣言に載せるかのどちらかにすること。**
