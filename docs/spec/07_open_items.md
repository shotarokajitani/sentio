...
## 「再連携のお願い」の差し込み2つが埋められない（**未判断・2026-09-08 登録**）

承認された文面（PS-S4）は **{会社名} / {検知日時 JST} / {再連携URL}** の3つを差し込む。
**検知日時は `connection_events` から引ける。残り2つは、いまのシステムに出所が無い。**

### 1. 会社名の正本が無い

- **`company_name` の列はどのマイグレーションにも無い**（`grep` で0件）
- 登録フォームが聞くのは**メールアドレス・パスワード・自社サイトURL（任意）だけ**
- 本番の実測（2026-09-08・読み取り照会）: `auth.users` は3件で、
  `raw_user_meta_data` のキーは `email` / `email_verified` / `phone_verified` / `sub`
  （1件だけ `subscription`）。**`site_url` は3件とも未設定**で、URL から導くこともできない
- `src/app/api/competitors/suggest/route.ts:28` にも
  「2026-09-02 まで `company_name` 必須だったが、登録時に聞いていない」と書かれている

### 2. 公開オリジンの設定が Edge Function に無い

`SITE_ORIGIN` を読む Edge Function は0件（`grep`）。再連携URLを組むには
**Edge の secrets に1つ設定が要る**（`SENTIO_SITE_ORIGIN`）。人が Dashboard で入れる作業になる。

### いまの実装（**送らずに止める**）

`renderReconnectNotice` は**差し込みが1つでも欠けたら `null` を返す**。
呼び出し側は 500 を返し、ディスパッチャが `failed_deliver` として記録して実行が non-2xx になる。
**「(不明) の連携が切れています」を顧客に送らない**ためで、これは fail-closed である。

**したがって、2つが埋まるまで「再連携のお願い」は1通も出ない。**
`§11-7` の6段のうち **(4)「翌朝1通届く」はここで止まる。**

**何を会社名とするかは判断しない。** 選択肢だけ挙げる——
登録時に会社名を聞く／メールアドレスのドメインから導く／文面から会社名を外す。
