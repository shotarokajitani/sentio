...
describe("CD-1: 対象の選び方", () => {
  it("CD-1-1: 連携があり宛先も取れる会社に run-sense → deliver-pulse を呼ぶ", async () => {
    ...
    expect(d.calls.map((c) => c.fn)).toEqual(["run-sense", "deliver-pulse"]);
    expect(d.calls[1].body).toMatchObject({ email: "owner@example.invalid" });
  });
...
describe("CD-2: 失敗の扱い", () => {
  ...
  it("CD-2-4: run-sense が失敗した会社にも deliver-pulse は走る（失敗としては数える）", async () => {
    ...
    expect(d.calls.map((c) => c.fn)).toEqual(["run-sense", "deliver-pulse"]);
    expect(result.status).not.toBe(200);
    expect(result.body).toMatchObject({ sense_failed: 1 });
  });
});
